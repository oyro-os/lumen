...
#[no_mangle]
pub extern "C" fn lumen_version_components(major: *mut u32, minor: *mut u32, patch: *mut u32) {
    if !major.is_null() {
        unsafe { *major = lumen::VERSION_MAJOR; }
    }
    if !minor.is_null() {
        unsafe { *minor = lumen::VERSION_MINOR; }
    }
    if !patch.is_null() {
        unsafe { *patch = lumen::VERSION_PATCH; }
    }
}
...
#[no_mangle]
pub extern "C" fn lumen_free_result(result: *mut LumenResult) {
    if !result.is_null() {
        unsafe {
...
pub extern "C" fn lumen_test_message() -> *mut c_char {
    CString::new("Hello from Lumen FFI!")
        .unwrap()
        .into_raw()
}
