...
LumenQueryBuilder lumen_query_create(LumenDatabase database, const char* table_name) {
    ...
    lumen_query_builder_t* query = (lumen_query_builder_t*)std::malloc(sizeof(lumen_query_builder_t));
...
char* lumen_query_to_sql(LumenQueryBuilder query) {
    if (!query || !query->table_name) return nullptr;
...
LumenTransaction lumen_transaction_begin(LumenDatabase database) {
    ...
    lumen_transaction_t* transaction = (lumen_transaction_t*)std::malloc(sizeof(lumen_transaction_t));
...
LumenResult lumen_transaction_commit(LumenTransaction transaction) {
    if (!transaction || !transaction->is_active) return LUMEN_ERROR_INVALID_ARGUMENT;
